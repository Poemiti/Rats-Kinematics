...
from utils.plot import plot_single_bodypart_trajectories, plot_3D_traj, plot_animation, plot_metric_time, annotate_single_bodypart
from utils.split_video import split_clip_range

from config import load_config
from utils.pipeline_maker import load_metrics, load_figure_maker, make_output_path

# ------------------------------------ setup ---------------------------------------

RAT_NAME = "#525"
cfg = load_config()
filenames, plot_choice = load_figure_maker(cfg, RAT_NAME)
        ...
        if trial["trial_success"] : 
            ...
            if plot_choice["plot_single_bodypart_trajectories"] : 
                ...
                ax = plot_single_bodypart_trajectories(
                    ...
                    frame_laser_on=int(trial["laser_on"] * 125) if trial["laser_on"] is not None else None,
                    color="blue",
                    transparancy=0.5
                )
            ...
            if plot_choice["annotate_single_bodypart"] :
            # --------------------------------------- single bodypart trajectory annotation ----------------------------------------------

                print(f"Making video annotation ...")
                
                annotate_single_bodypart(video_path=trial['filename_clips'],
                                        csv_path=trial["filename_coords"],
                                        output_path=make_output_path(output_fig_dir, "annotated_clips", f"anot_{trial_name}.png"),
